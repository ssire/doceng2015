...
//  PATCHED for AXEL compatibility by S. Sire (Oppidoc)
var axel_slidy = {
  special_key_element : function (node) { 
    return $(node).hasClass('pre') ? true : false;
  },
  special_mouse_element : function (node) { 
    return (($(node).closest('div.pre').length > 0) || ($(node).closest('div.mask').length > 0)) ? true : false;
  }
};
...
// invoked via F key
  ...
  key_down: function (event) {
    ...
    if (!w3c_slidy.slidy_chrome(target) &&
        !axel_slidy.special_key_element(target) && // AXEL compatibility by S. Sire
  ...
  mouse_button_click: function (e) {
    ...
    if (w3c_slidy.mouse_click_enabled && leftclick &&
        !w3c_slidy.special_element(target) &&
        !axel_slidy.special_mouse_element(target) && // AXEL compatibility by S. Sire
        !target.onclick)
    {
      w3c_slidy.next_slide(true);
      w3c_slidy.stop_propagation(e);
